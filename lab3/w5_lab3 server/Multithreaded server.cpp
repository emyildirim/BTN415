...
void Run(int Index) {
    ...
    while (true) {
        //TODO: receive the cordinates from the clients through threads
        // and display accordingly
        char RxBuffer[128] = { };
        memset(RxBuffer, 0, sizeof(RxBuffer));
        int x = recv(ClientSockets[Index], RxBuffer, sizeof(RxBuffer), 0);
        if (x == SOCKET_ERROR) {
            break;
        }else if(sizeof(RxBuffer) != 0)
        {
            std::cout << "From Thread " << Index << " : ";
            std::cout << RxBuffer << std::endl;
            send(ClientSockets[Index], "Ack", sizeof("Ack"), 0);
