...
//Lab 8 function
void prepare_packet(packet& packet, const char* header, int contentSize, bool flag, const char* content) {
    strncpy(packet.name, header, sizeof(packet.name) - 1);
    packet.name[sizeof(packet.name) - 1] = '\0';
    contentSize += 1;
    if (contentSize > sizeof(packet.letters) - 1) {
        packet.letters = new char[contentSize];
        packet.size = contentSize - 1;
    }
    else {
        packet.size = contentSize - 1;
        packet.letters = new char[packet.size + 1];
    }
    packet.flag = flag;
    strncpy(packet.letters, content, packet.size);
    packet.letters[packet.size] = '\0';
}
